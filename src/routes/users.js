...
router.get('/', 
  [
    ...
    query('role').optional().isIn(['app_admin', 'school_admin', 'teacher', 'clerk', 'user']).withMessage('Invalid role filter'),
...
router.put('/:id/role', 
  [
    param('id').isString().withMessage('Valid user ID is required'),
    body('role').isIn(['app_admin', 'school_admin', 'teacher', 'clerk', 'user']).withMessage('Invalid role'),
    validationMiddleware,
  ],
  jwtAuth,
